...
//////////////////////////// REAR WHEELS & PID ////////////////////////////
// Motors
const double PID_CONS_KP = 1.75;
const double PID_CONS_KI = 0.15;
const double PID_CONS_KD = 0.30;

volatile double PID_setpoint_right = 0;
volatile double PID_output_right = 0;
volatile double PID_input_right = 0;

volatile double PID_setpoint_left = 0;
volatile double PID_output_left = 0;
volatile double PID_input_left = 0;
...
#define ENCODER_PULSES_PER_ROTATION		1550
...
#ifdef MULTIPOINT_TURN
	#define STEERING_LEFT_EXTREME_DELTA		26
	#define STEERING_RIGHT_EXTREME_DELTA	38
#else
	#define STEERING_LEFT_EXTREME_DELTA		19
	#define STEERING_RIGHT_EXTREME_DELTA	20
#endif
...
#define STEERING_BUFFER_TIME_MS			500
	...
	#define TARGET_TURNING_RADIUS_CM		25
#else
	// Fixed Turning radius measured from the middle of the car body
	#define TARGET_TURNING_RADIUS_CM		41

#endif

#ifdef MULTIPOINT_TURN
	#define TURNING_LEFT_RADIUS_OFFSET_PER_90		4
	#define TURNING_RIGHT_RADIUS_OFFSET_PER_90		4
#else
	#define TURNING_LEFT_RADIUS_OFFSET_PER_90		-1
	#define TURNING_RIGHT_RADIUS_OFFSET_PER_90		1
#endif

// Distance between the front wheels, measured from the middle of each wheel
#define FRONT_WHEEL_TO_WHEEL_CM			17
#define TURNING_LEFT_OFFSET_PER_DEG				(TURNING_LEFT_RADIUS_OFFSET_PER_90 / 90)
#define TURNING_RIGHT_OFFSET_PER_DEG			(TURNING_RIGHT_RADIUS_OFFSET_PER_90 / 90)
// Radius of turning circle for inner wheel
#define	TURNING_INNER_WHEEL_RADIUS		(TARGET_TURNING_RADIUS_CM - (FRONT_WHEEL_TO_WHEEL_CM/2))
// Radius of turning circle outer inner wheel
#define	TURNING_OUTER_WHEEL_RADIUS		(TARGET_TURNING_RADIUS_CM + (FRONT_WHEEL_TO_WHEEL_CM/2))
...
#ifdef MULTIPOINT_TURN
	static const Movement_Command FL_CMDS[] = {
			{MOVE_FORWARD_LEFT, 0, 45},
			{MOVE_BACKWARD_RIGHT, 0, 47},
			{MOVE_FORWARD, 1, 0}
	};
	static const Movement_Command FR_CMDS[] = {
			{MOVE_FORWARD_RIGHT, 0, 45},
			{MOVE_BACKWARD_LEFT, 0, 47},
			{MOVE_FORWARD, 2, 0}
	};
	static const Movement_Command BR_CMDS[] = {
			{MOVE_BACKWARD_LEFT, 0, 40},
			{MOVE_FORWARD, 14, 0},
			{MOVE_FORWARD_RIGHT, 0, 40},
			{MOVE_BACKWARD_LEFT, 0, 40},
//			{MOVE_BACKWARD, 1, 0},
			{MOVE_BACKWARD_RIGHT, 0, 29},
			{MOVE_FORWARD, 5, 0}
	};
	static const Movement_Command BL_CMDS[] = {
			{MOVE_BACKWARD_RIGHT, 0, 40},
			{MOVE_FORWARD, 13, 0},
			{MOVE_FORWARD_LEFT, 0, 40},
			{MOVE_BACKWARD_RIGHT, 0, 40},
			{MOVE_BACKWARD_LEFT, 0, 28},
			{MOVE_FORWARD, 5, 0}
};
#endif
...
typedef uint16_t ULTRASONIC_DISTANCE_TYPE;
...
void command_to_encoder_counts(Movement_Command cmd);
/* USER CODE END PM */
...
// Create PIDs for rear motor movements
PID rightMotorPID(&PID_input_right, &PID_output_right, &PID_setpoint_right,
...
//			}
//			ULTRASONIC_DISTANCE_TYPE distance = (Ultrasonic_Difference * 0.034/2) + ULTRASONIC_OFFSET;
...
// Force the steering to move side to side in the hopes of
// ensuring that the steering will move to the center
void calibrate_steering_center(MOVEMENT_DIRECTION_TYPE direction)
{
...
#ifdef MULTIPOINT_TURN
// For a simple 90 degree turn, we actually doing a "3 point turn".
// So the single turn has to decomposed into several commands.
bool convert_turning_to_commands(Movement_Command cmd)
{
	MOVEMENT_DIRECTION_TYPE direction = cmd.command;

	if (direction == MOVE_FORWARD || direction == MOVE_BACKWARD || cmd.angle_deg != 90)
	{
		return false;
	}
	if (direction == MOVE_FORWARD_LEFT)
	{
		for (uint8_t i = 0; i < sizeof(FL_CMDS)/ sizeof(FL_CMDS[0]); i++)
		{
			// Put the command message into the queue
			osMessageQueuePut(movement_command_queue, &FL_CMDS[i], 0U, osWaitForever);
		}
		return true;
	}
	else if (direction == MOVE_FORWARD_RIGHT)
	{
		for (uint8_t i = 0; i < sizeof(FR_CMDS)/ sizeof(FR_CMDS[0]); i++)
		{
			// Put the command message into the queue
			osMessageQueuePut(movement_command_queue, &FR_CMDS[i], 0U, osWaitForever);
		}
		return true;
	}
	else if (direction == MOVE_BACKWARD_RIGHT)
	{
		for (uint8_t i = 0; i < sizeof(BR_CMDS)/ sizeof(BR_CMDS[0]); i++)
		{
			// Put the command message into the queue
			osMessageQueuePut(movement_command_queue, &BR_CMDS[i], 0U, osWaitForever);
		}
		return true;
	}
	else if (direction == MOVE_BACKWARD_LEFT)
	{
		for (uint8_t i = 0; i < sizeof(BL_CMDS)/ sizeof(BL_CMDS[0]); i++)
		{
			// Put the command message into the queue
			osMessageQueuePut(movement_command_queue, &BL_CMDS[i], 0U, osWaitForever);
		}
		return true;
	}
	return false;
...
void movement_task(void *argument)
{
  ...
  for(;;)
  {
	  if (!toMove)
	  {
		  ...
		  {
#ifdef MULTIPOINT_TURN
			  // Check if we need to decompose the turning command into smaller commands
			  if (convert_turning_to_commands(move_cmd))
			  {
				  toMove = false;
				  continue;
			  }
...
//	  // Delay for 10 us. This only works because only the ultrasonic is using timer 1
//	  __HAL_TIM_SET_COUNTER(&htim1, 0);
//	  while (__HAL_TIM_GET_COUNTER (&htim1) < 10);
//	  HAL_GPIO_WritePin(Ultrasonic_Trig_GPIO_Port, Ultrasonic_Trig_Pin, GPIO_PIN_RESET);
//
//	  // Enable interrupts with callback HAL_TIM_IC_CaptureCallback
